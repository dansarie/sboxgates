...
/* Target S-box. Change this to generate circuits for other S-boxes. */
const uint8_t g_target_sbox[] = {
...
int main(int argc, char **argv) {
  ...
  if (permute == 0) {
    memcpy(g_sbox_enc, g_target_sbox, 256 * sizeof(uint8_t));
  } else {
    for (int i = 0; i < 256; i++) {
      g_sbox_enc[i] = g_target_sbox[i ^ (uint8_t)permute];
    }
  }
